...
- 输出形式（可设定二维码版本 1-40）：
  ...
  - **动态二维码视频**：QR-transfer 协议（zfec 前向纠错）生成 MP4，
    ...
    手机摄像头对准屏幕即可传输；**每码显示时长可设（默认 0.33 秒，范围 0.2-2 秒）**，
    时长越长越易扫、总耗时线性变长；帧率与帧头协议不变，解析端完全兼容。
...
| POST | /encode | multipart：file 或 text + mode(static/video) + qr_version + raw(0/1) + display(秒，仅 video，默认 0.33) → task_id |
...
- 相机传输模式（默认）下视频时长为普通模式的 5 倍（帧率不变），超大文档建议调高二维码版本减少码数；
  每码显示时长可设（默认 0.33 秒）：调大更易扫但视频更长，调低下限 0.2 秒——
  低于此值手机摄像头每码采到的分析帧不足，识别率显著下降；
