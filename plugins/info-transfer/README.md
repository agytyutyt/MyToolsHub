...
- 输出形式（可设定二维码版本 1-40）：
  ...
  - **动态二维码流**：QR-transfer 协议（zfec 前向纠错），与「轨迹转换」
    ...
    手机摄像头对准屏幕即可传输；**每码重复帧数可设（默认 5 帧 ≈ 0.33 秒/码，范围 1-30 帧）**，
    帧数越多越易扫、总耗时线性变长（下限 1 帧为极限速度，实拍识别率要求高，日常建议 ≥3）；
    帧率与帧头协议不变，解析端完全兼容。
  - **MP4 按需生成**：默认**只产帧序列**（体积约为视频的 1/18，实测 v25 下
    6.7KB/码 vs 120KB/码；视频体积对分辨率还呈非单调跳变，落盘占用不可预估），
    需要把文件发给手机离线还原（「信息解析 - 二维码视频」）、给其他插件或归档时：
    封装时勾选「同时生成视频文件（MP4）」，或在结果卡点「生成视频文件并下载（MP4）」
    按需生成（像素取自已落盘且通过自检的帧 PNG，不重新渲染，≈11ms/帧）。
  - **全屏码面舞台**：静态与视频结果均提供「全屏展示 / 全屏播放（手机扫码）」
    ...
    任务结果（/task）新增 `qr_version` 字段供前端计算；全屏轮播与预览共用同一份
    帧窗口与游标，切全屏不重复下载。
...
- 每码重复帧数可设（默认 5 帧 ≈ 0.33 秒/码，范围 1-30 帧）：帧数越多越易扫、全程越久；
  ...
  重复帧数**不增加**磁盘体积（只产帧时 PNG 一码一张；出视频时实测 5→30 帧体积基本不变），
  只增加编码耗时与播放总时长；
