...
| 新增 / 升级插件 | **推荐**：用插件包——开发侧 `tools\build-plugin-package.ps1 -Id <插件id>` 出包，目标机二选一：① 双击「安装插件.bat」（离线脚本，免管理员，自动校验/备份/回滚）② 管理后台「插件管理」页上传 zip（免命令行、免解压，程序自动校验 → 备份 → 替换 → 停服重启，页面自动刷新）。见 `docs/离线部署包说明.md` §11；手工覆盖目录亦可，但需递增 `?v=` 与 `manifest.version` | 视改动而定（含后端改动则自动重启，约 5~10 秒） |
