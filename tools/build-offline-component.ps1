...
故障排查
  ...
  - 自检退出码 -1073741502（0xC0000142，DLL 初始化失败）、或 -1073741515（0xC0000135，找不到 DLL）：
    目标机的 VC++ 2015-2022 运行库**缺失或版本不匹配**。安装器会**自动**把组件自带的
    System64\ 里那套 DLL 补齐到 program\（日志里会出现 [补齐] 一行），通常这一步就解决了。
    仍不行时装一次 Microsoft Visual C++ 2015-2022 可再发行组件（x64）。
    注意：这种情况的表现是「1~2 秒退出、**没有任何输出**、profile 目录都不建」——
    先看退出码，别急着怀疑杀软。
