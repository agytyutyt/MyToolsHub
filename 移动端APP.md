...
| 四段覆盖 | 落点 |
| --- | --- |
| 接口契约 | **§3 协议规范（信封 / 三种码型判别 / 多页分片 / 视频帧协议 / 错误文案逐字契约）——四端齐改的一端**；§9 附录 A 协议速查卡 |
| 实现路径 | §4 一期实现指引 + §5 二期视频解析；**源码真源 `android-app/InfoParse/app/src/main/java/com/jztools/infoparse/`** |
| 当前实现状态 | §9 附录 E「与桌面端行为对照及修订记录」；已落地：JZ2 双协议解析（`Jz2.kt`）、k-of-m 早停、rawBytes 直读、xz 解压（`org.tukaani:xz`）、拆包重组还原、ImageDecoder 遍历全部码 |
| 后续优化方向 | `docs/plan/信息传输优化TODO清单.md`（信息传输唯一活清单）；APP 出包 → `android-app/InfoParse/InfoParse-APP打包手册.md` |

> **覆盖范围**：信息传输在 **APP 侧**的协议契约、实现指引、异常边界、测试与验收 —— 协议部分的**权威定义**
> ｜**不覆盖**：桌面端封装实现 → `plugins/info-transfer/README.md` 与 `docs/eval/信息传输格式开销评估与优化方案.md`；优化条目 → `docs/plan/信息传输优化TODO清单.md`
> ｜**随包**：是 ｜**最后核对**：2026-09-17

> ⚠️ **代码块的实现真源**：§4/§5 与附录 C 内嵌的 Kotlin 代码块是**离线参考快照**，
> 完整实现以 `android-app/InfoParse/app/src/main/java/com/jztools/infoparse/` 源码为准
> （受 Kotlin 单测约束，如 `Jz2Test` / `ZfecCompatTest` / `EnvelopeTest`）。
> **建议禁止据文档代码块直接改代码**；协议变更时按 `HANDOFF.md` §10.1 的「四端齐改」
> （桌面后端 + Web 前端 + APP + 本文档第 3 章）收尾，并同步本文附录代码块。
