...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (env.isExcel) {
            ...
            textPreview.text = "原始文件「${env.name}」已完整封装在二维码中。\n\n点击下方导出按钮即可还原原始文件（文件名与内容与原文档一致）。"
    ...
    /**
     * 导出内容字节（FR-05）：file → base64 还原原始文件；word（旧码）→ 重建 .docx；
     * excel → CSV；text/markdown → 原文 UTF-8。base64 解码失败返回 null。
     */
    private fun exportBytes(): ByteArray? = when {
        env.isFile -> env.fileBytes()
        env.isExcel -> Csv.fromRows(rows).toByteArray(Charsets.UTF_8)
        env.fmt == Fmt.WORD -> DocxWriter.fromText(env.textData ?: "")
        else -> (env.textData ?: "").toByteArray(Charsets.UTF_8)
    }

    /** file 按扩展名推断 mime，其余按格式导出规则 */
    private fun exportMime(): String =
        if (env.isFile) Exporter.fileMime(env.name) else Exporter.exportSpec(env.fmt).second
    ...
    private fun doExport() {
        ...
        val fileName = Exporter.exportFileName(env.fmt, env.name)
