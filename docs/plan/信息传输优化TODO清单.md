...
| 依据 | `docs/eval/信息传输格式开销评估与优化方案.md`（v1.3）+ `docs/archive/压缩算法评估（DS）.md` + `docs/plan/信息传输优化方案0917.md`（缺陷诊断 T16–T33）+ 本清单批次 10 的播放口径实测 |
| 决策说明 | **多码同屏方案经评审不采用**（`docs/archive/二维码多码同屏可行性评估.md` 相关结论未纳入本清单），全部任务按**单码口径**设计；原"静态 r×c 拼图""码位轮换"两项已移除（后者仅多码场景需要）。**二维码流播放口径（2026-09-17 定案）**：屏幕播放统一走帧序列 JS 轮播，MP4 改为按需生成（详见批次 10） |
| 用法 | 按批次顺序执行；每项含**任务名称 / 具体实现方案 / 预期效果**；完成后在状态列打勾并回写当日日志 |


| 四段覆盖 | 落点 |
| --- | --- |
| 接口契约 | 各批次的「具体实现方案」列 + §验收红线（贯穿所有批次） |
| 实现路径 | 改动面列（桌面端 / 两端同步）；实现集中在 `plugins/info-transfer/backend/routes.py` 与 `android-app/InfoParse` |
| 当前实现状态 | **条目状态以本文各批次的状态列为准，不在别处复述**；批次 1–5 主体已落地并真机验收，批次 6–10 见状态列 |
| 后续优化方向 | 本清单即「后续优化方向」的**唯一活清单**；批次 10 为二维码流播放口径（2026-09-17 定案） |

> **覆盖范围**：信息传输优化的**全部待办条目、批次划分、预期效果与验收红线**（唯一活清单）
> ｜**不覆盖**：收益口径 → `docs/eval/信息传输格式开销评估与优化方案.md`；缺陷诊断 → `docs/plan/信息传输优化方案0917.md`
...
> 批次 6–9 的**详细实施方案**（逐项改法、量化依据、验收脚本）见 `docs/plan/信息传输优化方案0917.md`；本清单只保留条目、预期效果与状态，避免两处重复维护。
...
5. 每批次发版按 `docs/guide/打包部署手册.md` 流程，先 commit 再打包；两端协议变更（批次 3/4）须同步《移动端APP.md》第 3 章与插件 README；
